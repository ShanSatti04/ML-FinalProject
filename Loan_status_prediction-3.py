...
if model is not None and label_encoders is not None and feature_names is not None:
    ...
    # Define numeric fields manually (must match training data)
    ...
    if st.button("🔍 Predict Loan Approval"):
        ...
        try:
            ...
            st.error(f"Input mismatch error: Missing field: {e}")
        except ValueError as e:
            st.error(f"ValueError during prediction: {e}")
else:
